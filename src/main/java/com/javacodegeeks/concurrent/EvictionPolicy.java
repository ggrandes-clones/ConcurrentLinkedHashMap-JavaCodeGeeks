...
public interface EvictionPolicy {
	 ...
	 * 
	 * <p>
	 * Invoking the <tt>get</tt> cache method results in an access to the corresponding entry (assuming it
	 * exists after the invocation completes). <i>No other methods generate entry accesses.</i> In particular,
	 * operations on collection-views do <i>not</i> affect the order of iteration of the backing cache.
	 * 
	 ...
	 * <tt>putAll</tt> method generates one entry insertion for each mapping in the specified map, in the
	 * order that key-value mappings are provided by the specified map's entry set iterator. <i>No other
	 * methods generate entry insertions.</i>
	 * 
	 ...
	 * this method's implementation returns a <i>not NULL<i> element. Invoking the <tt>put</tt> cache method
	 * results in an insertion of the corresponding entry.
	 ...
	 * 
	 * <p>
	 * This method has no effect if {@link #insertionOrder()} method is implemented to return false, whereas
	 * the newly inserted element will be placed at the end (just before the head) of the cache's double
	 * linked element list.
	 * 
	 * @param head the head of the double linked list of all elements in cache
